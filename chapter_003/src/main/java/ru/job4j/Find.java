...
public class Find {
    ...
    String runSearching(String in) {
        ...
        Validator validator = new Validator(in);

        if (!validator.validate()) {
            result.append("Not valid keys");
        } else {
            IOFileFilter fileFilter = new WildcardFileFilter(validator.name);
            Collection<File> listOfFiles = FileUtils.listFilesAndDirs(validator.dir, fileFilter, TrueFileFilter.TRUE);

            try {
                if (validator.log.exists()) {
                    FileUtils.forceDelete(validator.log);
                }
                ...
                FileUtils.writeStringToFile(validator.log, result.toString(), "utf-8");
            } catch (IOException e) {
                e.printStackTrace();
            }
        }
    ...
    /**
     * Class Validator contains keys of search.
     */
    private class Validator {
         ...
         * Constructor.
         *
         * @param keys string of keys.
         */
        Validator(String keys) {
            this.keys = keys;
        }
